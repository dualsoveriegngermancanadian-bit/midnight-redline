...
import { FEATURED_BUILD, UPGRADES, VEHICLES, performanceClass } from "@/game/raceData";
...
function GarageSheet({ snapshot, world, build, vehicleChosen, onVehicleSelected }: { snapshot: RaceSnapshot; world: GameWorld; build: { hp: number; weight: number; grip: number }; vehicleChosen: boolean; onVehicleSelected: () => void }) {
  ...
  return <>
    ...
    <div className="garage-hero"><img src={PLAYER_CAR_IMAGE} alt="Graphite turbo coupe in the player garage" /><div className="garage-hero-copy"><span>{snapshot.currentVehicle.tag}</span><h2>{snapshot.currentVehicle.name}</h2><p>{snapshot.currentVehicle.description}</p><b>{performanceClass({ horsepower: build.hp, weight: build.weight, eta: snapshot.currentVehicle.eta })} COMPETITION BAND</b>{snapshot.currentVehicle.id === FEATURED_BUILD.vehicleId && <small className="featured-build-note">STOCK {FEATURED_BUILD.stockHorsepower} HP → BUILT {FEATURED_BUILD.builtHorsepower} HP · ${formatCredit(FEATURED_BUILD.realWorldStockValueCents / 100)} STOCK REFERENCE · ${formatCredit(FEATURED_BUILD.engineAddOnInvestmentCents / 100)} BUILD INVESTMENT · {FEATURED_BUILD.topSpeedKph}+ KM/H · {FEATURED_BUILD.suspensionSensitivityLabel}</small>}</div></div>
